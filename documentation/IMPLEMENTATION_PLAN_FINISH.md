  # Implementation Plan — Akhtar Abbasi Hiking (Full-Stack Conversion + Polish)

**Project:** Convert static React Router v7 frontend to database-backed full-stack app, then add responsive design and polish
**Date:** 2026-07-04
**Total Phases:** 11

> **Phase progression:** Phases 1–7 cover the core full-stack conversion (Prisma → Auth → Public Routes → Admin CRUD → Image Upload → Video Embed → Deploy). Phases 8–11 cover hero admin control, responsive design, and UI polish.

### Phase summary

| Phase | Title | Status |
|-------|-------|--------|
| 1 | Prisma Schema + Supabase Setup + Seed Data | ✅ Complete |
| 2 | Authentication (Login, JWT, Admin Middleware) | ✅ Complete |
| 3 | Public Routes (Static Data → Prisma Loaders) | ✅ Complete |
| 4 | Admin CRUD Routes | ✅ Complete |
| 5 | Image Upload to Supabase Storage | ✅ Complete |
| 6 | Video URL Embed | 🔄 In progress |
| 7 | Deploy to Vercel | 🔄 In progress |
| **8** | **Hero Slides Admin Control** | **🔄 In progress** |
| **9** | **Public Page Responsiveness** | **❌ Not started** |
| **10** | **Admin Dashboard Responsiveness** | **❌ Not started** |
| **11** | **Logo Sizing Fix** | **❌ Not started** |
...
- [ ] `prisma generate` succeeds and produces `HeroSlide` type
- [ ] `GET /admin/hero` lists hero slides with edit/delete actions
- [ ] `GET /admin/hero/new` renders the creation form with ImageInput
- [ ] `POST /admin/hero/new` creates a slide and redirects
- [ ] `GET /admin/hero/:id/edit` pre-fills the form with existing data
- [ ] `POST /admin/hero/:id/edit` updates the slide
- [ ] Delete action removes the slide and redirects
- [ ] Home page hero carousel reads slides from the database (not hardcoded)
- [ ] All 3 default slides exist after re-seeding
- [ ] "Hero" link appears in the admin sidebar
- [ ] `npm run typecheck` passes
